...
`laya` is a multilingual, non-autoregressive System 1 decision engine: one forward pass (~33 ms) emits many **typed decisions** (`choice | score | noul`), `Router` picks the right checkpoint per request, trained with RLCD/proper scoring, with hooks for audit. We adapt the *pattern* to browser control without shipping `laya`:
