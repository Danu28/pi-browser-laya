...
export function formatSnapshot(snap: Snapshot): string {
  ...
  if ((snap as any).crossOriginSkipped)
    lines.push(
      `Note: ${(snap as any).crossOriginSkipped} cross-origin iframe(s) skipped (CORS, opaque) — not pierceable, expected.`
    );
  if ((snap as any).closedShadowSkipped)
    lines.push(
      `Note: ${(snap as any).closedShadowSkipped} potential closed shadow root(s) skipped (mode:closed, opaque by spec) — not pierceable, expected.`
    );
  ...
  lines.push("");
  lines.push(
    `=== PAGE TEXT (12k, ALL visible incl. offscreen) ${truncated ? `(page is ${len} chars, truncated)` : ""} ===`
  );
  ...
  if (!snap.actions || snap.actions.length === 0) lines.push("(no interactive elements visible)");
    ...
    for (const a of snap.actions) {
      ...
      const combobox = (a as any).combobox
        ? ` combobox="${String((a as any).combobox).slice(0, 40)}"`
        : "";
      const vmsg = a.validationMessage
        ? ` validation:"${String(a.validationMessage).slice(0, 60)}"`
        : "";
      lines.push(
        `[${a.id}] ${role.padEnd(10)} ${label.slice(0, 80)}${val}${extra} (${kind})${y}${vis}${frame}${fillHint}${disabled}${checked}${selected}${expanded}${combobox}${vmsg}`
      );
    }
  lines.push("");
  lines.push("=== GENERAL WORKFLOW (any site, any form, shadow/iframe) ===");
  lines.push(
    "1) browser_launch → full snapshot (12k text + element table with y/frame + fill/disabled/validation). Offscreen/shadow/iframe (open) listed and clickable."
  );
  lines.push(
  ...
  lines.push(
    '4) For long-page/beyond-12k: browser_text {"query":"<phrase>"} case-insensitive | {"offset":-5000} | {"blockIndex":10} | {"offset":0,"limit":25000} joined view'
  );
  lines.push(
